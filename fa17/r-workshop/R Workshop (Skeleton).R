...
grapes <- FALSE
my_fruits <- apples + oranges
more_fruits <- apples + bananas
even_more_fruits <- oranges + grapes
...
movie_names <- c('sorcerer', 'chamber', 'prisoner', 'goblet', 
...
# arrange
titanic3 <- arrange(titanic2, age)
titanic3 <- arrange(titanic2, desc(fare))
...
titanic4 <- arrange(filter(titanic2, embarked=='Q', sex=='female'), 
                    desc(age))

#Display the passenger ID and number of family members of females who survived
select(filter(titanic2, sex=='female', survived==1), 
       passenger_ID, fam)

#Display the percentage of survival and average age for each class
summarize(group_by(titanic2, class), 
          mean(survived), mean(age))
...
summarize(group_by(titanic2, sex, class), 
          mean(survived), median(age), mean(fare))
#--------------------------------------------------------------------
